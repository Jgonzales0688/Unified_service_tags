...
import (
	...
	"gitlab.com/heb-engineering/teams/platform-engineering/gke-hybrid-cloud/kon/apps/Mutating-Webhook/pkg/admission"
...
// ServeMutatePods returns an admission review with the mutated pod as the response
func ServeMutatePods(w http.ResponseWriter, r *http.Request) {
	...
	// Return the mutated pod as the response
	w.Header().Set("Content-Type", "application/json")
	jout, err := json.Marshal(mpod)
	if err != nil {
		e := fmt.Sprintf("could not parse admission response: %v", err)
		logger.Error(e)
		http.Error(w, e, http.StatusInternalServerError)
